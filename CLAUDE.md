...
**Runtime schema patches** are applied in `applyCustomResolvers` and therefore reflected in `schema.graphql` when regenerated. Examples: `ApiKey.scopes` is patched from `String!` to `[String!]!` to correct drizzle-graphql's handling of the `text[].array()` column, and `ApiKey.keyHash` is deleted from the type entirely so the stored token hash can never be selected.
